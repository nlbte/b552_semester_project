# B552 Semester Project — Results Report
...
motif_analysis.py           extract 14 graph features, run Mann-Whitney U tests
        |
        v
extended_analysis.py        correlation matrix, logistic regression, motif scoring,
                             out-degree breakdown, error magnitude analysis, case studies
        |
stats_tests.py              Fisher's exact + logistic regression with odds ratios
        |
visualize_results.py        accuracy/token plots
visualize_graphs.py         20-graph grid visualizations per model
```
...
### 1. Incorrect traces are structurally larger — consistently across both models
Incorrect answers use more steps (median 9 vs. 7) and have deeper reasoning chains (median 5 vs. 4). This is the strongest and most replicated signal. Both models show it at high significance.

### 2. Depth is the single most predictive feature
Significant in both models' Mann-Whitney tests, both standardized logistic regressions, and both odds-ratio regressions. The odds ratio (0.664 for gemma4, 0.673 for gpt-oss) is nearly identical — a ~33% reduction in correctness odds per unit of added depth. This cross-model replication is compelling.

### 3. Fact extraction fraction is positively predictive
Correct traces spend a higher proportion of steps explicitly pulling information from the question (`frac_facts`). Incorrect traces rush past fact extraction into computation. This is consistent across both models.

### 4. frac_arithmetic is NOT a useful predictor — or is slightly positive
Both models dedicate a similar fraction of steps to arithmetic operations whether they get the answer right or wrong. What matters is not *how much arithmetic* is done but *how well it's structured*.

### 5. Orphan nodes predict failure in gpt-oss (not as strongly in gemma4)
Dead-end steps — nodes that don't feed into any downstream reasoning — are significantly more common in gpt-oss's incorrect traces (0.244 vs. 0.088 mean). This is weaker in gemma4 (0.128 vs. 0.062), but directionally consistent. gpt-oss's verbosity means orphaned exploration branches are more visible in the graph.

### 6. Errors split cleanly into catastrophic and near-miss
Catastrophic failures (>1000x off) tend to come from fundamental misinterpretation — the model applies the right procedure to the wrong problem. Near misses tend to be single arithmetic errors in an otherwise correct chain. These require different interventions.

### 7. Models never make unsupported conclusions
`unsupported_conclude` = 0 across all 200 traces for both models. Both models always trace their final answer back to some prior reasoning step, even in catastrophic failures. The reasoning graph is never "conclusion from nowhere."

### 8. The structural signals generalize across models
The most important result: the same features predict failure in both gpt-oss and gemma4, at nearly identical magnitudes. This is evidence that the structural properties of reasoning traces are genuine signals about reasoning quality, not model-specific artifacts.

### 9. Off-schema steps are the strongest single predictor
`has_offschema_node` — any step labeled outside the five-category schema — achieves Fisher p=3.97e-09 (gpt-oss) and p=1.34e-08 (gemma4), with logit ORs of 0.085 and 0.077 respectively. It remains dominant in extended regressions controlling for depth, n_steps, and frac_facts. This is the highest-significance finding in the project.

### 10. Verbose ungrounded traces replicate as a failure signal
`verbose_ungrounded` (n_steps >= 8 AND frac_facts < 0.3) replicates across both models (gpt-oss logit OR=0.133, p=1.21e-06; gemma4 logit OR=0.300, p=0.0035). This is best understood as a threshold combination of the two already-significant features rather than an independent signal.

### 11. Verification is a distress signal, not a quality-control habit
Traces with `verify` steps are more common in incorrect traces (gpt-oss: 39% incorrect vs. 23% correct, p=0.029; gemma4: 21% vs. 15%, p=0.26). Models invoke verification reactively on hard problems rather than as routine rigor.

### 12. Step count (not depth) survives the within-problem control
In 14 discordant pairs where one model was right and the other wrong on the same problem, `n_steps` is significantly higher in incorrect traces (Wilcoxon p=0.023). `depth` is not significant (p=0.338), suggesting depth is partly driven by problem difficulty while step count reflects reasoning quality.
...
| File | Description |
|---|---|
| `gsm_hard_traces.jsonl` | Raw model outputs: thinking trace, answer text, predicted answer, correctness, token counts |
| `gsm_hard_graphs_all.jsonl` | Combined DAG graphs for all 200 problems |
| `graph_features.csv` | 14 structural features per graph |
| `feature_comparison.csv` | Mann-Whitney U results |
| `logistic_regression.csv` | Standardized logistic regression |
| `logistic_odds_ratios.csv` | Odds-ratio regression |
| `motif_scores.csv` | Per-trace motif flags and composite count |
| `error_split.csv` | Per-trace gold/predicted/relative error |
| `outdegree_by_op.csv` | Node-level out-degree by op type |
...
| File | Description |
|---|---|
| `run_meta.json` | Run config: model, temperature, timestamp, n_prompts=200 |
| `gsm_hard_traces.jsonl` | Raw model outputs |
| `gsm_hard_graphs.jsonl` | DAG graphs |
| `graph_features.csv` | 14 structural features per graph |
| `feature_comparison.csv` | Mann-Whitney U results |
| `logistic_regression.csv` | Standardized logistic regression |
| `logistic_odds_ratios.csv` | Odds-ratio regression |
| `motif_scores.csv` | Motif flags and composite count |
| `error_split.csv` | Error magnitude data |
| `outdegree_by_op.csv` | Out-degree by op type |
| `targeted_motifs_gemma4.csv` | Three targeted motif flags per trace |
| `additional_motifs_gemma4.csv` | Five additional motif flags per trace |
| `correlation_matrix.png` | Spearman correlation heatmap |
| `feature_comparison.png` | Correct vs. incorrect boxplots |
| `motif_score_distribution.png` | Motif count distribution by correctness |
| `outdegree_by_op.png` | Out-degree boxplots by op type |
| `error_distribution.png` | log10 error histogram |
| `case_studies.png` | Example DAG visualizations (2 correct, 2 incorrect) |
| `graph_grids/gemma4_correct_20.png` | Grid of 20 correct gemma4 reasoning graphs |
| `graph_grids/gemma4_incorrect_20.png` | Grid of 20 incorrect gemma4 reasoning graphs |
...
- `anthropic` — Anthropic API (gsm_hard_claude.py)
- `datasets` — HuggingFace dataset loader
- `pandas`, `numpy` — data handling
- `matplotlib` — all plots
- `networkx` — DAG construction and graph metrics
- `scipy` — Mann-Whitney U, Fisher's exact, point-biserial correlation
- `statsmodels` — logistic regression with p-values
- `python-dotenv` (optional) — `.env` loading for API keys
